...
import api from '../../services/api.js'; // Conexão com a API para buscar dados da OSC
import styles from './OSCDashboard.module.css';

// Ícones para os Cards e Banners
const FileIcon = () => (
...
// Ícones do Escudo de Governança
...
export default function OSCDashboard() {
  ...
  // Busca dados da OSC para o Escudo de Governança e Calendário Inteligente
  useEffect(() => {
  ...
  // --- Cálculo do Escudo de Governança ---
  ...
  // --- Lógica da Data de Origem (Para acinzentar meses anteriores à criação) ---
  const originDateStr = oscData?.data_origem_estatuto || oscData?.data_fundacao || oscData?.created_at;
  let originYear = 2000;
  let originMonth = 0; // 0 = Jan
  ...
  // Determina o status visual de cada mês
  const getMonthConfig = (index) => {
    const monthNum = index + 1;

    // Regra 1: Antes da criação da OSC -> Fica Cinza (Inativo)
    if (selectedYear < originYear || (selectedYear === originYear && index < originMonth)) {
        return { status: '-', class: styles.statusFuturo, isBlocked: true };
    }
    
    // Regra 2: Anos anteriores ativos -> Tudo o que não está 'Concluso' (ainda não temos a API de docs aqui) é 'Atraso'
    if (selectedYear < currentYear) {
      return { status: 'ATRASO', class: styles.statusAtraso, isBlocked: false };
    }

    // Regra 3: Ano vigente (Ex: 2026)
    if (selectedYear === currentYear) {
      if (monthNum < currentMonth) {
        if (monthNum === currentMonth - 1 && currentDay <= 10) {
           return { status: 'ABERTO', class: styles.statusAberto, isBlocked: false };
        }
        return { status: 'ATRASO', class: styles.statusAtraso, isBlocked: false };
      }
      if (monthNum === currentMonth) {
  ...
  const handleMonthClick = (m, monthIndex) => {
    if (m.isBlocked) return; // Não clica se for antes da origem
  ...
  return (
    <div className={styles.container}>
      ...
      {/* 1.5. BANNER DE GOVERNANÇA (FASE 3) */}
      <Link to="/osc/perfil" style={{ textDecoration: 'none' }}>
        <div style={{ 
          display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 20px', 
          backgroundColor: governanceStatus.bg, border: `1px solid ${governanceStatus.border || '#d1d5db'}`, 
          borderRadius: '8px', marginBottom: '24px', color: governanceStatus.color,
          boxShadow: '0 1px 3px rgba(0,0,0,0.05)', transition: 'transform 0.2s', cursor: 'pointer'
        }}>
          <div style={{ padding: '8px', backgroundColor: 'rgba(255,255,255,0.5)', borderRadius: '50%' }}>
            {governanceStatus.icon}
          </div>
          <div style={{ flex: 1 }}>
        ...
        <div className={styles.card}>
          ...
          <div className={styles.cardContent}>
            <div className={styles.labelWithInfo}>
              ...
              <div className={styles.tooltipContainer}>
                <InfoIcon />
                <span className={styles.tooltipText}>Acesse a aba 'Meus Documentos' para regularizar.</span>
              </div>
            </div>
        ...
        <div className={styles.monthsGrid}>
          {months.map((m, index) => (
            <div 
              ...
              style={{ 
                cursor: m.isBlocked || m.status === '-' ? 'default' : 'pointer',
                opacity: m.isBlocked ? 0.3 : 1
              }}
              title={m.isBlocked ? "Mês anterior à criação da OSC" : ""}
            >
